...
if not args.use_baseline:
    sys.stdout = open(
        './spn_output/testvgcndim=' + str(args.dim) + '_spndata_m=' + str(m) +
        "vocab=" + cfg_vocab_adj
        +"threshold="+str(args.threshold)+ args.model_name +
        "len=" + str(args.seq_len) + '.txt', 'w')
else:
    sys.stdout = open('./spn_output/testvgcn_output_b4edata_baseline' + args.model_name + '.txt', 'w')
...
data_dir = f"gen_spn_seq/data_train_spn"
output_dir = "spn_output/"

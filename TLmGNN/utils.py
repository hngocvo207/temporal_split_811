...
def sparse_scipy2torch(coo_sparse):
    ...
    return torch.sparse_coo_tensor(i, v, torch.Size(coo_sparse.shape))
...
def example2feature(
    ...
    # do not need use bert.tokenizer again, because be used at gen_train_MulDi.py
...
class CorpusDataset(Dataset):
    ...
    def pad(self, batch):
        ...
        batch_confidences = torch.tensor(np.array(f_collect(3)), dtype=torch.float)
        ...
        batch_guid = torch.tensor(f_collect(6), dtype=torch.long)

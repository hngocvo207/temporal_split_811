...
if not args.use_baseline:
    sys.stdout = open(
        './MulDi_output/testvgcndim=' + str(args.dim) + '_zsedata_m=' + str(m) +
        "vocab=" + cfg_vocab_adj
        +"threshold="+str(args.threshold)+ args.model_name +
        "len=" + str(args.seq_len) + '.txt', 'w')
else:
    sys.stdout = open('./output/testvgcn_output_zsdata_baseline' + args.model_name + '.txt', 'w')
...
data_dir = f"data_train_MulDi"
output_dir = "MulDi_output/"
